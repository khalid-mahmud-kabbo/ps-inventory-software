...
// This new function calculates the balance change for TODAY ONLY.
public function getPartyDailyMovement($partyIds)
{
    if (empty($partyIds)) {
        return ['balance' => 0, 'status' => ''];
    }
    ...
    $today = today();
    $partyIdsString = implode(',', array_map('intval', $partyIds));

    // Define the date filter closure for Eloquent queries for TODAY
    $todayFilter = function ($query) use ($today) {
        // CRITICAL CHANGE: Use whereDate to filter for TODAY exactly
        $query->whereDate('created_at', $today);
    };

    // Define the date condition string for raw SQL WHERE clauses for TODAY
    $dateCondition = " AND DATE(t1.created_at) = '" . $today->toDateString() . "'";
    $paymentDateCondition = " AND DATE(payment_transactions.created_at) = '" . $today->toDateString() . "'";


    // NOTE: Opening Balance is inherently calculated across all time,
    // but for daily movement, we only need the daily transactions.
    // Assuming Opening Balance is zero for a single day's movement.
    $openingBalance = 0;


    // 1. PartyPayment (Receive Sum) - Transactions created TODAY ONLY
    $partyPaymentReceiveSum = PartyPayment::whereIn('party_id', $partyIds)
        ->where('payment_direction', 'receive')
        ->selectRaw("
            (SELECT SUM(amount) FROM party_payments AS t1
             WHERE party_id IN ({$partyIdsString}) AND payment_direction = 'receive' {$dateCondition})
            -
            COALESCE(
                (SELECT SUM(payment_transactions.amount)
                 FROM payment_transactions
                 INNER JOIN party_payment_allocations
                     ON payment_transactions.id = party_payment_allocations.payment_transaction_id
                 WHERE party_payment_allocations.party_payment_id IN
                     (SELECT id FROM party_payments AS t2
                      WHERE party_id IN ({$partyIdsString}) AND payment_direction = 'receive' AND DATE(t2.created_at) = '" . $today->toDateString() . "')
                 {$paymentDateCondition}
                ), 0)
        AS total_amount")
        ->value('total_amount') ?? 0;

    // 2. PartyPayment (Pay Sum) - Transactions created TODAY ONLY
    $partyPaymentPaySum = PartyPayment::whereIn('party_id', $partyIds)
        ->where('payment_direction', 'pay')
        ->selectRaw("
            (SELECT SUM(amount) FROM party_payments AS t1
             WHERE party_id IN ({$partyIdsString}) AND payment_direction = 'pay' {$dateCondition})
            -
            COALESCE(
                (SELECT SUM(payment_transactions.amount)
                 FROM payment_transactions
                 ...
                 WHERE party_payment_allocations.party_payment_id IN
                     (SELECT id FROM party_payments AS t2
                      WHERE party_id IN ({$partyIdsString}) AND payment_direction = 'pay' AND DATE(t2.created_at) = '" . $today->toDateString() . "')
                 {$paymentDateCondition}
                ), 0)
        AS total_amount")
        ->value('total_amount') ?? 0;

    // 3. Sales and Payments - Documents created TODAY ONLY
    $saleBalance = Sale::whereIn('party_id', $partyIds)
        ->when(true, $todayFilter) // <--- APPLIED FILTER FOR TODAY
        ->selectRaw('coalesce(sum(grand_total - paid_amount), 0) as total')
        ->value('total');

    // 4. Sale Return and Payments - Documents created TODAY ONLY
    $saleReturnBalance = SaleReturn::whereIn('party_id', $partyIds)
        ->when(true, $todayFilter) // <--- APPLIED FILTER FOR TODAY
        ->selectRaw('coalesce(sum(grand_total - paid_amount), 0) as total')
        ->value('total');

    // 5. Purchase and Payments - Documents created TODAY ONLY
    $purchaseBalance = Purchase::whereIn('party_id', $partyIds)
        ->when(true, $todayFilter) // <--- APPLIED FILTER FOR TODAY
        ->selectRaw('coalesce(sum((grand_total - shipping_charge) -
                         CASE
    ...
    // 6. Purchase Return and Payments - Documents created TODAY ONLY
    $purchaseReturnBalance = PurchaseReturn::whereIn('party_id', $partyIds)
        ->when(true, $todayFilter) // <--- APPLIED FILTER FOR TODAY
        ->selectRaw('coalesce(sum(grand_total - paid_amount), 0) as total')
        ->value('total');

    // Final Calculation Logic for TODAY's Movement
    $balance = $openingBalance - ($partyPaymentReceiveSum - $partyPaymentPaySum);
    $balance += ($saleBalance) - ($saleReturnBalance);
    $balance -= ($purchaseBalance) - ($purchaseReturnBalance);

    // Determine the nature of today's NET movement
    if ($balance > 0) {
        // Today's activity created a net receivable
        return ['balance' => $balance, 'status' => 'net_receivable_today'];
    } elseif ($balance < 0) {
        // Today's activity created a net payable
        return ['balance' => abs($balance), 'status' => 'net_payable_today'];
    } else {
        return ['balance' => 0, 'status' => 'no_net_movement'];
    }
}
