...
public function exportDatabase()
{
    ...
    foreach ($tables as $table) {
        ...
        $createTableStmt = preg_replace('/NOT NULL DEFAULT \'0000-00-00 00:00:00\'/i', 'DEFAULT NULL', $createTableStmt);
        $createTableStmt = str_replace("'0000-00-00 00:00:00'", "NULL", $createTableStmt);
        
        $sqlDump .= "DROP TABLE IF EXISTS `$tableName`;\n";
        $sqlDump .= $createTableStmt . ";\n\n";
        $primaryKeyInfo = DB::select("SHOW KEYS FROM `$tableName` WHERE Key_name = 'PRIMARY'");
        $primaryKey = !empty($primaryKeyInfo) ? $primaryKeyInfo[0]->Column_name : null;
        $query = DB::table($tableName);

        if ($primaryKey) {
            $query->chunkById(100, function ($rows) use (&$sqlDump, $tableName, $pdo) {
                foreach ($rows as $row) {
                    $values = array_map(function ($value) use ($pdo) {
                        if (is_null($value) || $value === '0000-00-00 00:00:00') return 'NULL';
...
{
    $request->validate(['sql_file' => 'required|mimes:sql,txt']);
    ...
    try {
        ...
        $sql = preg_replace('/NOT NULL DEFAULT \'0000-00-00 00:00:00\'/i', 'DEFAULT NULL', $sql);
        ...
        return response()->json([
            'status' => true,
            'message' => 'Database permanently fixed and imported.'
        ]);

    } catch (\Exception $e) {
        Schema::enableForeignKeyConstraints();
        Log::error("Import Failure: " . $e->getMessage());
