$(function() {
    ...
    */
    window.loadDatatables = function() {
        ...
        table.on('xhr', function() {
            ...
            if (json && json.cash_in_hand_total !== undefined) {
                const total = parseFloat(json.cash_in_hand_total);
                const el = $('#cashInHandTotal');
                el.text(total);
    ...
    */
    ...
    function afterSeccessOfAjaxRequest(formObject, response) {
        if (response.cash_in_hand_total !== undefined) {
            const total = parseFloat(response.cash_in_hand_total);
            const el = $('#cashInHandTotal');
            el.text(total);
            el.removeClass('text-success text-danger')
              .addClass(total >= 0 ? 'text-success' : 'text-danger');
